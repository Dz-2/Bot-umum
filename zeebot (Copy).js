...
switch (command) {      
    ...
    case 'help': case 'menu': case 'helep':
          ...
          let hasil = `${res[0].isi}\n~${res[0].by}`
          menu =`${p}# Hallo ${pushname}${p}    
${p}• BOT STATE :${p}
${p}• Nama Owner : ${Miminnya}${p}
${p}• Nama Bot : ${BotName}${p}
${p}• Mode : ${publik ? 'Public' : 'Self'}${p}
${p}• Runtime : ${runtime(process.uptime())}${p}
${p}• Total Chat : ${totalchat.length}${p}
${p}• Hit Today : ${hit_today.length} Hit${p}
${p}• Total Hit : ${totalhit} Hit${p}
${p}• Browser : ${zee.browserDescription[1]}${p}
${p}• Server : ${zee.browserDescription[0]}${p}
${p}• Version : ${zee.browserDescription[2]}${p}
${p}• Handphone : ${zee.user.phone.device_manufacturer}${p}
${p}• Speed : ${process.uptime()}${p}
${p}• Versi Whatsapp : ${zee.user.phone.wa_version}${p}`
          zee.sendMessage(from, { contentText: menu, footerText: `zeebotz22@gmail.com`,buttons: [{ buttonId: `!allmenu`, buttonText: { displayText: '</menu>' }, type: 1 },{ buttonId: `!info`, buttonText: { displayText: '</info>' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: tamnel, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
          break
